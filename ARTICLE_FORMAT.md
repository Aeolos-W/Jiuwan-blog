...
方框内部**支持所有 markdown 行内格式**，包括：

```
> Theorem 1: 设 **P** 为平面上的点集，则存在...
> 详见 [Wikipedia 页面](https://en.wikipedia.org/wiki/...) 的讨论。
> 定义映射 *f*: R^2 -> R^2 如下...
```

- `**加粗**` — 加粗文字
- `*斜体*` — 斜体文字
- `[链接文字](url)` — 超链接（自动在新标签页打开）
- `` `代码` `` — 行内代码
- `![图片描述](url)` — 图片
...
你可以像引用公式一样，在正文中引用并跳转到任意方框。

**定义方框**（带编号）：
```
> 历史溯源 1: 《九章算术》中最早记载了这一方法。
```

**引用方框**：
```
如 (历史溯源 1) 中所述，这一方法源远流长...
```

**效果**：`(历史溯源 1)` 会变成蓝色可点击链接，点击后跳转到对应方框。

**不带编号的方框**也可以引用：
```
> 历史溯源: 《九章算术》中最早记载了这一方法。
```

后文写 `(历史溯源)` 即可跳转。

> **注意**：只有正文中写出的 `(名称 编号)` 或 `(名称)` 与方框定义**完全一致**时，才会生成链接。

---

### 5. 跳转到文章任意位置（自定义锚点）

除了公式和方框的自动引用，你还可以在文章的**任意位置**插入自定义锚点，并通过链接跳转到该位置。

#### 方式一：标题自动锚点

所有标题（`#`、`##`、`###`）都会**自动生成 id**，你可以直接引用。
...
**id 生成规则**：标题文字去除标点、空格替换为连字符。例如：
- `## 历史背景` → id 为 `历史背景`
- `## Main Results` → id 为 `main-results`
- `## 1. 引言` → id 为 `1-引言`

#### 方式二：自定义锚点 {#id}

如果你想在非标题位置（比如某段文字中间）设置锚点，使用 `{#id}` 语法：
...
| 写法 | 识别为 | 跳转到 | 条件 |
|------|--------|--------|------|
| `(Theorem 1)` | 方框引用 | Theorem 1 方框 | 文章中有 `> Theorem 1: ...` |
| `(历史溯源)` | 方框引用 | 历史溯源方框 | 文章中有 `> 历史溯源: ...` |
| `(1)` | 公式引用 | 编号公式 (1) | 文章中有 `$$...$$ (1)` |
| `（1）` | 普通文本 | 不跳转 | 中文括号，始终不跳转 |

**如何避免误识别？**

如果你只是想写普通括号数字，不希望它变成链接：

1. **使用中文括号**：`（1）`、（2） —— 永远不会被识别为引用
2. **确保没有对应定义**：如果文章中没有 `$$...$$ (1)` 也没有 `> 名称 1: ...`，那么 `(1)` 就是普通文本
3. **使用文字描述**：比如"第一点"代替 `(1)`
...
> 历史溯源 1: 这一问题最早可追溯到1946年Erdos的奠基性论文。他在文中不仅提出了单位距离问题，还同时提出了著名的不同距离问题，后者已由 **Guth 和 Katz** 在2015年基本解决。详见 [Guth-Katz 论文](https://arxiv.org/abs/1011.4105)。

The optimality of Theorem 1 is a heavily modified version of a result of Cilleruelo.

由式 (1) 可以看出，单位距离的计数与复数单位圆上的点分布密切相关。如 (历史溯源 1) 中所述，这一问题已有近80年的历史。
